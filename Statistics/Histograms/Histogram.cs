    ...
    public class Histogram: IHistogram, IReportMessage
    {
        ...
        public Int64 EstimateIterationsRemaining(double upperq, double lowerq)
        {
            ...
            if (ufxp > 0.0 & uxp != 0)
            {
                ...
                if (estimate > Int64.MaxValue)
                {
                    upperestimate = Int64.MaxValue;
                } else
                {
            ...
            if (lfxp > 0.0 & uxp != 0)
            {
                double estimate = Math.Ceiling(val * (Math.Pow((lz2 / (lxp * _ConvergenceCriteria.Tolerance * lfxp)), 2.0)));
                if (estimate > Int64.MaxValue)
                {
                    lowerestimate = Int64.MaxValue;
                } else
                {
