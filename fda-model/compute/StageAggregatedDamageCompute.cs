...
namespace fda_model.compute
{
    public class StageAggregatedDamageCompute
    {
        ...
        private void Compute(int seed, int iterations)
        {
            ...
            List<ConsequenceResultsUncertainPairedData> ConResultUPDList = new List<ConsequenceResultsUncertainPairedData>();// this is a list of histogram uncertain paired data for each index catagory, damage catagory, asset catagory.
            ...
            for (int c = 0; c < stagesForCompute.Length; c++)
            {
                ...
                for (int i = 0; i < iterations; i++)
                {
                    ...
                    foreach (string impactAreaDamCat in aggregatedResults.Keys)
                    {
                        ...
                        if (ConResultUPDList.Count == 0)
                        {
                            CreateEmptyHistograms(stagesForCompute, smallestEventDepths, ref ConResultUPDList, impactArea, damcatName);
                        }
                        else
                        {
                            bool foundHistogram = false;
                            foreach (ConsequenceResultsUncertainPairedData conResultsUncertainPD in ConResultUPDList)
                            {
                                if (conResultsUncertainPD.CurveMetaData.Name == impactArea && conResultsUncertainPD.DamageCategory == damcatName)
                                {
                                    foundHistogram = true;
                                    switch (conResultsUncertainPD.AssetCategory)
                                    {
                                        case "Structure":
                                            conResultsUncertainPD.Yvals[c].AddObservationToHistogram(aggregatedResults[impactAreaDamCat].StructureDamage, i);
                                            break;
                                        case "Content":
                                            conResultsUncertainPD.Yvals[c].AddObservationToHistogram(aggregatedResults[impactAreaDamCat].ContentDamage, i);
                                            break;
                                        case "Other":
                                            conResultsUncertainPD.Yvals[c].AddObservationToHistogram(aggregatedResults[impactAreaDamCat].OtherDamage, i);
                                            break;
                                    }
                                }
                            }
